...
def render_js():
    JS('(function(){'
      ...
      'if(twCurrent===target){twDeleting=true;twTimer=setTimeout(typeStep,2600);return;}'
      'twTimer=setTimeout(typeStep,140);'
      '}else{'
      'twCurrent=twCurrent.slice(0,-1);'
      'tw.textContent=twCurrent;'
      'if(twCurrent.length===0){twDeleting=false;twIdx=(twIdx+1)%words.length;twTimer=setTimeout(typeStep,450);return;}'
      'twTimer=setTimeout(typeStep,85);'
      '}'
      '}'
